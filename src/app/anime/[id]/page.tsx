...
// Extracted: detail view for a found anime (no try/catch wrapper)
function AnimeDetailView({ anime }: { anime: AnimeItem }) {
  return (
...
// Extracted: error fallback (no try/catch wrapper)
function AnimeDetailError() {
  return (
 ...
 *
 * Data fetching is in try/catch, but JSX is rendered outside it to satisfy
 * the react-hooks/error-boundaries ESLint rule.
 */
export default async function AnimeDetailPage({ params }: AnimeDetailPageProps) {
  const { id } = await params;

  let anime: AnimeItem | undefined;
  let fetchError = false;

  try {
    const allAnime = await fetchAnimeData();
    anime = allAnime.find((item) => item.id === id);
  } catch (error) {
